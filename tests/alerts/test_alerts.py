...
from tests.alerts.alerts_data import ALERT_PAYLOAD, ALERT_UPDATES, UPDATED_ALERT, ALERTS_LIST, FILTERED_ALERTS
...
def test_update_alert_payload(get_session):
    alerts = Alerts(get_session, TENANT)
    assert alerts.update_alert_payload(ALERT_PAYLOAD, ALERT_UPDATES) == UPDATED_ALERT


def test_get_filtered_alerts_by_group(get_session):
    alerts = Alerts(get_session, TENANT)
    assert alerts.get_filtered_alerts_by_group(ALERTS_LIST, GROUP_NAME) == FILTERED_ALERTS


@patch.object(Alerts, "get_alert_config")
def test_get_alert_config(mock_get_alert_config,get_session):
...
def test_update_alert(mock_update_alert,get_session):
...
def test_update_alert_group(mock_update_alert_group,get_session):
...
def test_fetch_alerts_data(mock_fetch_alerts_data,get_session):
...
def test_delete_alert(mock_delete_alert,get_session):
...
def test_list_alerts(mock_list_alerts,get_session):
...
def test_get_alert_dataframe(mock_get_alert_dataframe,get_session):
    mock_df = MagicMock()
