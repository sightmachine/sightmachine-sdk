...
def test_update_alert_payload(get_session:Any)->None:
    alerts = Alerts(get_session, TENANT or "genmills-ms-rbg")
...
def test_get_alert_config(mock_get_alert_config:MagicMock, get_session:Any)->None:
    mock_get_alert_config.return_value = ALERT_PAYLOAD

    alerts_instance = Alerts(get_session, TENANT or "genmills-ms-rbg")  # Create an instance
    assert alerts_instance.get_alert_config(1) == ALERT_PAYLOAD


@patch.object(Alerts, "update_alert")
def test_update_alert(mock_update_alert:MagicMock, get_session:Any)-> None:
...
def test_update_alert_group(mock_update_alert_group:MagicMock, get_session:Any)->None:
    mock_update_alert_group.return_value = True
    alerts_instance = Alerts(get_session, TENANT or "genmills-ms-rbg")
    assert alerts_instance.update_alert_group(GROUP_NAME, {"priority": "high"})


@patch.object(Alerts, "fetch_alerts_data")
def test_fetch_alerts_data(mock_fetch_alerts_data:MagicMock, get_session:Any)->None:
...
def test_delete_alert(mock_delete_alert:MagicMock, get_session:Any) ->None:
...
def test_list_alerts(mock_list_alerts:MagicMock, get_session:Any) ->None:
...
def test_get_alert_dataframe(mock_get_alert_dataframe:MagicMock, get_session:Any) ->None:
    mock_df = MagicMock()
